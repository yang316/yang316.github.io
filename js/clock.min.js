    ...
    fetch(`https://restapi.amap.com/v3/ip?key=${gaud_map_key}`).then(res=>res.json()).then(data=>{
        ...
        fetch(`https://devapi.qweather.com/v7/weather/now?location=${data.rectangle.split(";")[0]}&key=${qweather_key}`).then(res2=>res2.json()).then(data2=>{
            if (document.getElementById("hexo_electric_clock")) {
                ...
                var week = ["SUN", "MON", "TUE", "WED", "THU", "FRI", "SAT"], card_clock_loading_dom;
                function updateTime() {
                    var cd = new Date, card_clock_time = zeroPadding(cd.getHours(), 2) + ":" + zeroPadding(cd.getMinutes(), 2) + ":" + zeroPadding(cd.getSeconds(), 2), card_clock_date = zeroPadding(cd.getFullYear(), 4) + "-" + zeroPadding(cd.getMonth() + 1, 2) + "-" + zeroPadding(cd.getDate(), 2) + " " + week[cd.getDay()], card_clock_dackorlight = cd.getHours(), card_clock_dackorlight_str;
                ...
                var timerID = setInterval(updateTime, 1e3);
                updateTime()
            }
        }
        )
